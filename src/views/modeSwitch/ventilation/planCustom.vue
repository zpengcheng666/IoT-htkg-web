...
import AddPlanCustom from '@/views/modeSwitch/ventilation/components/addPlanCustom.vue'
...
function getList() {
  ...
  api.get('/ventilation/planCustom/list', postData).then((res: any) => {
...
function clickEdit(row: any) {
  ...
  api.get('/ventilation/planCustom/queryById', postData).then((res: any) => {
    if (!res.result) {
      ElMessage({
        type: 'error',
        message: '数据不存在',
...
function addSubmit(val: any) {
  ...
  if (data.dialogType === 'add') {
    api.post('/ventilation/planCustom/add', postData).then((res: any) => {
      if (res.code === 200) {
        ElMessage({
          type: 'success',
          message: res.message,
    ...
    api.post('/ventilation/planCustom/edit', postData).then((res: any) => {
      if (res.code === 200) {
        ElMessage({
          type: 'success',
          message: res.message,
...
function del(id: string) {
  ...
  ElMessageBox.confirm(
    ...
    .then(() => {
      api.delete(`/ventilation/planCustom/delete?id=${id}`).then((res: any) => {
        if (res.code === 200) {
          ElMessage({
            type: 'success',
            message: res.message,
...
<template>
  <page-main class="pageBox">
    ...
    >
      <template #deviceNames="{ item }">
        <el-tag v-for="v in item.row.deviceNames" :key="v" type="success">
          {{ v }}
        </el-tag>
      </template>
      <template #operate="{ item }">
        <el-button type="primary" :icon="Edit" :dark="true" @click="clickEdit(item.row)" />
        <el-button type="danger" :icon="Delete" :dark="true" @click="del(item.row.id)" />
      </template>
    </CommonTable>
    <AddPlanCustom
      v-if="dialogAdd"
