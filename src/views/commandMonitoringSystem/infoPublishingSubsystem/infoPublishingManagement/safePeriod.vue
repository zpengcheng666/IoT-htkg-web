 ...
 * @LastEditTime: 2024-05-17 17:32:48
-->

<script lang="ts" setup name="PersonalSetting">
import { ElMessage, ElMessageBox } from 'element-plus'
import { Plus } from '@element-plus/icons-vue'
import addSafePeriod from './components/addSafePeriod.vue'
import publishDialog from './components/publishDialog.vue'
import importSafePeriod from './components/importSafePeriod.vue'
import api from '@/api'
...
function getList() {
  ...
  api.get('/message/publishSatelliteSecure/list', postData).then((res: any) => {
...
function del(id: string) {
  ...
  ElMessageBox.confirm(
    ...
    .then(() => {
      api.delete(`/message/publishSatelliteSecure/delete?id=${id}`).then((res: any) => {
        if (res.code === 200) {
          ElMessage({
            type: 'success',
            message: res.message,
...
function addSubmint(val: any) {
  ...
  if (data.dialogType === 'add') {
    api.post('/message/publishSatelliteSecure/add', postData).then((res: any) => {
      if (res.code === 200) {
        ElMessage({
          type: 'success',
          message: res.message,
    ...
    api.post('/message/publishSatelliteSecure/edit', postData).then((res: any) => {
      if (res.code === 200) {
        ElMessage({
          type: 'success',
          message: res.message,
...
function publishSubmint(val: any) {
  const postData = val
  api.put('/message/publishSatelliteSecure/publish', postData).then((res: any) => {
    if (res.code === 200) {
      ElMessage({
        type: 'success',
        message: res.message,
...
function cancelPublish(id: any) {
  ElMessageBox.confirm(
    ...
    .then(() => {
      api.put(`/message/publishSatelliteSecure/revocation/${id}`).then((res: any) => {
        if (res.code === 200) {
          ElMessage({
            type: 'success',
            message: res.message,
...
function importSubmint(val: any) {
  // console.log(val)
  const postData = val
  api.post('/message/publishSatelliteSecure/saveBatch', postData).then((res: any) => {
    if (res.code === 200) {
      ElMessage({
        type: 'success',
        message: res.message,
...
function exportXls() {
  ElMessageBox.confirm(
    ...
    .then(() => {
      ...
      api.get('/message/publishSatelliteSecure/exportXls', postData).then((data: any) => {
        const blobOptions = { type: 'application/vnd.ms-excel' }
        const fileSuffix = '安全时段.xls'
    ...
    <page-main class="pageBox">
      <el-row>
        ...
        <el-col :span="8" class="col-right">
          <el-button v-auth="'ips:safePeriod:import'" type="primary" @click="openImportDialog()">
            导入
          </el-button>
          <el-button v-auth="'ips:safePeriod:export'" type="primary" @click="exportXls()">
            导出
          </el-button>
          <el-button v-auth="'ips:safePeriod:add'" type="warning" :icon="Plus" @click="clickAdd()">
            新增
          </el-button>
        </el-col>
      </el-row>
      ...
      >
        ...
        <template #operation="{ item }">
          <el-button v-auth="'ips:safePeriod:edit'" :disabled="item.row.state === 1 || item.row.state === 2" type="primary" size="small" @click="clickEdit(item.row)">
            编辑
          </el-button>
          <el-button v-if="item.row.state === 0" v-auth="'ips:safePeriod:publish'" type="primary" plain size="small" @click="clickPublish(item.row)">
            发布
          </el-button>
          <el-button v-if="item.row.state === 1 || item.row.state === 2" v-auth="'ips:safePeriod:publish'" size="small" @click="cancelPublish(item.row.id)">
            撤销
          </el-button>
          <el-button v-if="item.row.state === 3" type="primary" size="small" disabled>
            已过期
          </el-button>
          <el-button v-auth="'ips:safePeriod:del'" type="danger" size="small" @click="del(item.row.id)">
            删除
          </el-button>
        </template>
      </CommonTable>
      <!-- 新增弹窗 -->
      <addSafePeriod v-if="dialogAdd" :title="data.dialogTitle" :type="data.dialogType" :dialog-visible="dialogAdd" :info-data="data.infoData" @close-dialog="closeDialog" @submint-dialog="addSubmint" />
      ...
      <!-- 导入弹窗 -->
      <importSafePeriod v-if="dialogImport" :dialog-visible="dialogImport" @close-dialog="closeImportDialog" @submit-dialog="importSubmint" />
    </page-main>
  </div>
</template>
