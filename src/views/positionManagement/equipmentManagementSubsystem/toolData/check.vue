 ...
 * @LastEditTime: 2024-05-14 14:54:55
-->
<script lang="ts" setup name="PersonalSetting">
import { ElMessage, ElMessageBox } from 'element-plus'
import { ref } from 'vue'
import addCheckDialog from './components/addCheckDialog.vue'
...
function getList() {
  ...
  api.get('spare/check/page', postData).then((res: any) => {
...
function addSubmit(val: any) {
  ...
  if (data.dialogType === 'add') {
    api.post('spare/check/add', postData).then((res: any) => {
      if (res.code === 200) {
        ElMessage({
          type: 'success',
          message: res.message,
    ...
    api.post('spare/check/edit', postData).then((res: any) => {
      if (res.code === 200) {
        ElMessage({
          type: 'success',
          message: res.message,
...
function getNo() {
  ...
  api.get('spare/check/getNo', postData).then((res: any) => {
...
function getInfo(id: any) {
  api.get('spare/check/getById/' + id).then((res: any) => {
...
<template>
  <div>
    <page-main class="pageBox">
      ...
      <!-- 新增弹窗 -->
      <addCheckDialog v-if="dialogAdd" :title="data.dialogTitle" :dialog-type="data.dialogType" :dialog-visible="dialogAdd" :info-data="data.infoData" @submit-dialog="addSubmit" @close-dialog="closeDialog" />
    </page-main>
  </div>
</template>
