 ...
 * @LastEditTime: 2024-05-09 15:19:45
-->
...
function addUser(user: any) {
  // 检查用户是否已经存在于数组中
  let exists = infoDataForm.items.some((u: { id: any }) => u.id === user.id);
...
function clickTree(val: any) {
  setTimeout(() => {
    ...
    infoDataForm.items[data.selectIndex].areaId = val.id
    infoDataForm.items[data.selectIndex].warehouseId = val.warehouseId
  }, 100);
...
<template>
  <el-dialog v-model="dialogShow" :title="props.title" width="50%" :before-close="closeDialog">
    <el-form ref="ruleFormRef" :rules="rules" :inline="true" :model="infoDataForm" class="demo-form-inline"
      label-width="100px">
      <el-form-item label="入库单号" placeholder="请输入" prop="orderNo">
        <el-input v-model="infoDataForm.orderNo" disabled class="formInput" />
      </el-form-item>
      <el-form-item label="入库类型" placeholder="请输入" prop="orderType">
        <el-select v-model="infoDataForm.orderType" filterable clearable class="formInput" :disabled="props.dialogType === 'info'">
          <el-option v-for="item in props.typesOptions" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="供应商" placeholder="请输入" v-if="infoDataForm.orderType===11">
        <el-select v-model="infoDataForm.supplierId" filterable clearable class="formInput" :disabled="props.dialogType === 'info'">
          <el-option v-for="item in props.supplierOptions" :key="item.id" :label="item.carrierName" :value="item.id" />
        </el-select>
      </el-form-item>
      ...
      <CommonTable :table-data="infoDataForm.items" :table-header="tableHeader" :is-select="false" :is-page="false">
        ...
        <template #areaId="{ item }">
          <el-tree-select v-if="props.dialogType === 'add'" v-model="item.row.areaId" clearable default-expand-all
    ...
    <selectItemDialog v-if="dialogAddItem" :dialog-visible="dialogAddItem" dialog-type="receipt" :info-data="infoDataForm.items"
