...
import addZpxxInformation
  from "@/views/positionManagement/equipmentManagementSubsystem/equipmentManagement/components/addZpxxInformation.vue";
...
function getList() {
  ...
  api.get('/deviceManager/zpxx/list', postData).then((res: any) => {
...
function del(id: string) {
  ...
  ElMessageBox.confirm(
    ...
    .then(() => {
      api.delete(`/deviceManager/zpxx/delete?id=${id}`).then((res: any) => {
        if (res.code === 200) {
          ElMessage({
            type: 'success',
            message: res.message,
...
function exportXls() {
  ElMessageBox.confirm(
    ...
    .then(() => {
      ...
      api.get('/deviceManager/zpxx/exportXls', postData).then((data: any) => {
        const blobOptions = { type: 'application/vnd.ms-excel' }
        const fileSuffix = '转盘信息.xls'
...
function addSubmit(val: any) {
  ...
  if (data.dialogType === 'add') {
    api.post('/deviceManager/zpxx/add', postData).then((res: any) => {
      if (res.code === 200) {
        ElMessage({
          type: 'success',
          message: res.message,
    ...
    api.post('/deviceManager/zpxx/edit', postData).then((res: any) => {
      if (res.code === 200) {
        ElMessage({
          type: 'success',
          message: res.message,
    ...
    <page-main class="pageBox">
      <el-row>
        ...
        <el-col :span="4" class="col-right">
          ...
          <el-button v-auth="'zpxx:equipment:add'" type="warning" small @click="clickAdd()">
            新增
          </el-button>
        </el-col>
      </el-row>
...
<!--          </el-button>-->
          ...
          <el-button v-auth="'zpxx:equipment:edit'" type="primary" size="small" @click="clickEdit(item.row)">
            编辑
          </el-button>
          <el-button v-auth="'zpxx:equipment:del'" type="danger" size="small" @click="del(item.row.id)">
            删除
          </el-button>
        </template>
      </CommonTable>
      <!-- 新增弹窗 -->
      <addZpxxInformation v-if="dialogAdd" :location-tree-data="data.locationTreeData" :tree-data="data.treeData" :title="data.dialogTitle" :dialog-type="data.dialogType" :dialog-visible="dialogAdd" :info-data="data.infoData" @close-dialog="closeDialog" @submit-dialog="addSubmit" />
