...
import handDuty from './components/handDuty.vue'
...
const data = reactive({
  ...
  componentList: [
    ...
    { terminalName: '手动值班', infoType: 'handDuty', imgUrl: zb },
...
const bgStyle = reactive({
  width: '100%',
  height: '100%',
  backgroundImage: `url(${data.terminalInfo.backgroundImg})`,
  backgroundColor: data.terminalInfo.backgroundColor,
...
// 获取模板下拉
const getTemplateOptiongs = () => {
  const param = {}
  api.get('system/baseTerminalTemplate/templateOptiongs', param).then((res: any) => {
    data.templateOptiongs = res.result
  }).catch((error) => {
    window.console.log(error)
  })
}
...
// 选择模板
function selectTemplateFun(val: any) {
  const imgArr = []
  if (val.backgroundImg) {
    imgArr.push(val.backgroundImg)
  }
  data.imgUrls = imgArr
  data.layoutList = val.infoList
  data.terminalInfo.name = val.name
  data.terminalInfo.backgroundColor = val.backgroundColor
  data.terminalInfo.backgroundImage = val.backgroundImg
  data.terminalInfo.yjcz = val.yjcz
  data.terminalInfo.mj = val.mj
  data.terminalInfo.ajm = val.ajm
  data.terminalInfo.alarmStatus = val.alarmStatus
  data.terminalInfo.checkDoorId = val.checkDoorId
  data.terminalInfo.doorId = val.doorId
  data.terminalInfo.alarmLevel = val.alarmLevel
  bgStyle.backgroundColor = val.backgroundColor
  bgStyle.backgroundImage = `url(${val.backgroundImg})`
}

// 获取图片
function onSuccess(val: any) {
  data.imgUrls.push(val.result)
  data.terminalInfo.backgroundImg = val.result
  bgStyle.backgroundImage = `url(${val.result})`
}
function update(val: any) {
  data.imgUrls = val
  data.terminalInfo.backgroundImg = ''
...
async function saveBigScreen(formEl: FormInstance | undefined) {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      postData.infoList = data.layoutList
...
function getInfo(id: any) {
  api.get(`system/baseTerminal/queryById?id=${id}`).then((res: any) => {
    const imgArr = []
    if (res.result.backgroundImg) {
      imgArr.push(res.result.backgroundImg)
    }
    data.imgUrls = imgArr
    data.layoutList = res.result.infoList
    data.terminalInfo = res.result
    bgStyle.backgroundColor = res.result.backgroundColor
    bgStyle.backgroundImage = `url(${res.result.backgroundImg})`
  })
}
              ...
              >
                ...
                >
                  ...
                  <div style="height: 100%;">
                    ...
                    <handDuty v-if="item.infoType === 'handDuty'" />
          ...
          <el-tab-pane label="基本信息" name="1">
            <el-form ref="ruleFormRef" :model="data.terminalInfo" :rules="rules" class="demo-form-inline" label-width="80px">
              ...
              <el-form-item label="编码" placeholder="请输入" class="formInput" prop="code">
                <el-input v-model="data.terminalInfo.code" />
              </el-form-item>
              <el-form-item label="位置" placeholder="请输入" class="formInput" prop="locationId">
                <el-tree-select v-model="data.terminalInfo.locationId" clearable default-expand-all node-key="id" check-strictly :props="defaultProps" :data="data.treeData" />
              </el-form-item>
              <el-form-item label="模板" placeholder="请输入" class="formInput">
                <el-select v-model="data.selectTemplate" filterable clearableplaceholder="请选择" @change="selectTemplateFun">
                  <el-option
                    v-for="item in data.templateOptiongs"
                    :key="item.id"
                    :label="item.name"
                    :value="item"
                  />
              ...
              <el-form-item label="背景颜色" placeholder="请输入" class="formInput">
                <el-color-picker v-model="data.terminalInfo.backgroundColor" show-alpha @change="selectBgColor" />
              </el-form-item>
              ...
              <el-form-item label="描述" placeholder="请输入" class="formInput">
                <el-input v-model="data.terminalInfo.description" />
              </el-form-item>
            </el-form>
